  ...
  const currentUrl = window.location.href;  // Defin currentUrl as the user's current browser URL
  const noProto = currentUrl.replace(/(^\w+:|^)\/\//, '');  // Remove the http(s):// protocol from that URL
  const url = setSheetUrl(url);  // Define the variable url as: the function setSheetUrl() and passing the variable url through it.
  //console.log(noProto);
